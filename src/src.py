...
#Proyecto final: Software para reconocimiento de notas musicales para estudio de voz con tensoflow

Proyecto final de Procesamiento del Habla de TSCDIA ISPC perteneciente a Julio Mariano Sachi
"""
...
for file in file_list:
    ...
    audio_data, sr = librosa.load("/content/drive/MyDrive/Trumpet_C/TpC-ord-G5-pp-N-T25d.wav", sr=None)
...
test_audio, sr = librosa.load('/content/drive/MyDrive/Trumpet_C/TpC-ord-C4-pp-N-T10d.wav', sr=None)

if len(test_audio) > max_length:
    processed_audio = test_audio[:max_length]
else:
    processed_audio = np.pad(test_audio, (0, max_length - len(test_audio)), mode='constant')

predictions = loaded_model.predict(np.expand_dims(processed_audio, axis=0))
predicted_class_index = np.argmax(predictions[0])

print('Etiqueta predicha:', predicted_class_index)